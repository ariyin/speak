...
function Analysis() {
  ...
  const [deliveryData, setDeliveryData] = useState<DeliveryAnalysis | null>(null);
  ...
  useEffect(() => {
    const analyze = async () => {
      try {
        ...
        if (deliveryAnalysis) { setDeliveryData(deliveryAnalysis); }
        if (contentAnalysis) { setContentData(contentAnalysis); }
        ...
        if (analysisType.includes("delivery")) {
          ...
          if (t_response.status !== 200) {
            throw new Error(`Error analyzing transcript! status: ${t_response.status}`);
          }
          ...
          if (b_response.status !== 200) {
            throw new Error(`Error analyzing transcript! status: ${b_response.status}`);
          }
        ...
        if (d_update_response.status !== 200) {
          throw new Error(`Error updating delivery analysis! status: ${d_update_response.status}`);
        }
        ...
        if (c_update_response.status !== 200) {
          throw new Error(`Error updating content analysis! status: ${c_update_response.status}`);
        }
        ...
        <div className="rounded-2xl border-2 border-black p-4 space-y-4">
          <h2>Delivery Analysis</h2>
            {deliveryData?.transcript_analysis ? (
              <>
                <details className="border rounded-lg p-2">
                  <summary className="cursor-pointer text-lg font-medium">Filler Words</summary>
                  <ul className="pl-4 list-disc">
                    {deliveryData?.transcript_analysis?.analysis?.filler_words &&
                    Object.keys(deliveryData.transcript_analysis.analysis.filler_words).length > 0 ? (
                      Object.entries(deliveryData.transcript_analysis.analysis.filler_words).map(
                        ([word, count], idx) => ( <li key={idx}> {word}: {count} </li> )
                      )
                    ) : (
                      <li>None found</li>
                    )}
                ...
                <details className="border rounded-lg p-2">
                  <summary className="cursor-pointer text-lg font-medium">Speaking Rate</summary>
                  <p className="pl-4">{deliveryData.transcript_analysis.analysis?.speech_rate_wpm ?? "N/A"}</p>
                </details>
              </>
            ) : (
            ...
            {deliveryData?.body_language_analysis ? (
              <details className="border rounded-lg p-2 mb-4">
                <summary className="cursor-pointer text-lg font-medium">Body Language</summary>

                <details className="border rounded-lg p-2 mt-2 ml-4">
                  <summary className="cursor-pointer font-semibold">Pros</summary>
                  <ul className="pl-4 list-disc max-h-48 overflow-y-auto">
                    {deliveryData.body_language_analysis.pros.length > 0 ? (
                      deliveryData.body_language_analysis.pros.map(({ timestamp, description }, idx) => (
                        <li key={idx}>
                ...
                <details className="border rounded-lg p-2 mt-2 ml-4">
                  <summary className="cursor-pointer font-semibold">Areas of Improvement</summary>
                  <ul className="pl-4 list-disc max-h-48 overflow-y-auto">
                    {deliveryData.body_language_analysis.cons.length > 0 ? (
                      deliveryData.body_language_analysis.cons.map(({ timestamp, description }, idx) => (
                        <li key={idx}>
                          <strong>{timestamp}</strong>: {description}
                        </li>
                      ))
